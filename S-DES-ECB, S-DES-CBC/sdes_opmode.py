...
def sdes_encrypt_ecb(text: bitarray, key: bitarray):
    ...
    for i in range(0,len(text),8):
        result+=sdes(text[i:i+8],key,1)
    return result

def sdes_decrypt_ecb(ciphertext: bitarray, key: bitarray):
    result=bitarray('')
    for i in range(0,len(ciphertext),8):
        result+=sdes(ciphertext[i:i+8],key,2)
    return result
    pass
...
def sdes_encrypt_cbc(text: bitarray, key: bitarray, iv:bitarray):
    result = bitarray('')
    # iv bit수 맞추기
    iv=bitarray('0'*(8-len(iv)))+iv
    tempiv=iv
    for i in range(0, len(text), 8):
        temptext=tempiv ^ text[i:i + 8]
        tempresult = sdes(temptext, key, 1)
        result+=tempresult
        tempiv=tempresult
    return result
    pass
...
def sdes_decrypt_cbc(ciphertext: bitarray, key: bitarray, iv:bitarray):
    ...
    # iv bit수 맞추기
    iv = bitarray('0' * (8 - len(iv))) + iv
    tempresult = sdes(ciphertext[0:8], key, 2)
    result+=tempresult ^ iv
    for i in range(8,len(ciphertext),8):
        tempresult=sdes(ciphertext[i:i+8],key,2)
        result+=tempresult ^ ciphertext[i-8:i]
    return result
    pass
